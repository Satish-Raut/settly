...
const ImportWizard = () => {
  ...
  const { stagedExpenses, anomalies, importSession, isParsed, loading, error } =
    useSelector((state) => state.import);
  const group = useSelector((state) =>
    state.groups.groups.find((g) => g.id === state.groups.selectedGroupId),
  );
  ...
  const handleFinalizeImport = async () => {
    ...
    stagedExpenses.forEach((staged) => {
      // splitWith is stored as a semicolon-separated string of names (e.g. "Aisha;Rohan")
      // It's already in the format the backend expects — pass it through directly
      const splitWithStr = Array.isArray(staged.splitWith)
        ? staged.splitWith
      ...
      const splitDetailsStr =
        staged.splitDetails && typeof staged.splitDetails === "object"
          ? Object.entries(staged.splitDetails)
              .map(([uid, val]) => {
                const u = users.find((user) => user.id === parseInt(uid));
                return u
                  ? `${u.username} ${val}${staged.splitType === "percentage" ? "%" : ""}`
                  : "";
              })
              .filter(Boolean)
              .join(";")
    ...
    <div className="w-full bg-white text-slate-800 font-sans pb-16">
      ...
      <div className="max-w-4xl mx-auto px-8 mt-10">
        <h2 className="text-3xl font-extrabold font-serif text-slate-900 tracking-tight">
          CSV Import Resolution Wizard
        </h2>
        <p className="text-slate-500 mt-2 text-sm">
          Scan for spreadsheet anomalies, duplicates, and timeline errors.
          Interactively approve all data updates before committing to the database.
        </p>
        ...
        {!isParsed && !loading && (
          <div className="mt-8 border-2 border-dashed border-slate-300 rounded-2xl p-12 bg-white flex flex-col items-center justify-center text-center hover:border-brand transition-colors">
            ...
            <h3 className="font-bold font-serif text-slate-800 text-lg mb-2">
              Upload expenses_export.csv
            </h3>
            <p className="text-xs text-slate-500 mb-6 max-w-sm">
              Upload the raw spreadsheet export. Our validation pipeline checks
              12 specific classes of anomalies.
            </p>
            <input
              type="file"
              ref={fileInputRef}
        ...
        {loading && (
          <div className="mt-12 text-center">
            ...
            <p className="text-slate-500 text-sm mt-4">
              Analyzing spreadsheet columns and rows...
            </p>
          </div>
        )}
          ...
          <div className="mt-8 space-y-8 animate-fade-in">
                ...
                <div className="flex gap-4 mt-2">
                  <span className="text-xs font-semibold text-slate-600">
                    <strong className="text-slate-900">
                      {stagedExpenses.length}
                    </strong>{" "}
                    total rows parsed
                  </span>
                  <span className="text-xs font-semibold text-emerald-600">
                    <strong className="text-emerald-700">
                      {stagedExpenses.filter((e) => !e.isFlagged).length}
                    </strong>{" "}
                    clean rows
                  </span>
              ...
              <div className="flex flex-col items-end gap-2">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={finalizing}
                    className="px-4 py-2 border border-slate-200 rounded-lg text-xs font-bold text-slate-500 hover:bg-slate-50 transition-colors cursor-pointer disabled:opacity-50"
                  >
                    Cancel Import
                  </button>
                  <button
                    onClick={handleFinalizeImport}
                    disabled={getUnresolvedAnomalies().length > 0 || finalizing}
                    className="px-5 py-2 text-xs font-bold text-white bg-emerald-600 disabled:bg-slate-100 disabled:text-slate-400 hover:bg-emerald-700 active:bg-emerald-800 rounded-lg transition-colors shadow-xs cursor-pointer flex items-center gap-2"
                  >
                    {finalizing ? (
                      <><span className="animate-spin inline-block w-3 h-3 border-2 border-white border-t-transparent rounded-full"></span> Saving to DB...</>
                    ) : (
                ...
                {finalizeError && (
                  <div className="bg-rose-50 border border-rose-200 rounded-lg px-4 py-2 text-xs text-rose-700 font-semibold max-w-sm text-right">
                    ❌ {finalizeError}
                  </div>
                )}
              </div>
            ...
            <div>
              <h3 className="text-base font-bold font-serif text-slate-800 mb-4 flex items-center gap-2">
                ⚠️ Surfaced Anomalies ({getUnresolvedAnomalies().length} Action
                Required)
              </h3>
              ...
              {getUnresolvedAnomalies().length === 0 ? (
                <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-6 text-center text-emerald-800 text-sm font-semibold">
                  🎉 All anomalies successfully resolved! You are ready to
                  complete the import and download the report.
                </div>
              ) : (
                <div className="space-y-4">
                  {anomalies
                    ...
                    .map((anomaly) => {
                      const row = stagedExpenses.find(
                        (s) => s.id === anomaly.stagedExpenseId,
                      );
                      if (!row) return null;
                        ...
                        >
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs bg-slate-50 p-3 rounded-lg border border-slate-100">
                            <div>
                              ...
                              <span className="font-bold block mt-0.5 text-slate-700">
                                Line {row.rowNumber}
                              </span>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <span className="font-bold block mt-0.5 text-slate-700">
                                {row.originalAmount} {row.currency || "INR"}
                              </span>
                            </div>
                            <div>
                              <span className="text-slate-400 font-medium uppercase text-[9px]">
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              {anomaly.type === "DUPLICATE_ROW" && (
                                <>
                                  <button
                                    onClick={() =>
                                      selectResolutionChoice(
                                        anomaly,
                                        "discard",
                                        `Discarded duplicate Row ${row.rowNumber} in favor of Row ${anomaly.duplicateOfRow}`,
                                        { isExcluded: true },
                                      )
                                    }
                                  ...
                                  >
                                    Discard Row {row.rowNumber} (Keep Row{" "}
                                    {anomaly.duplicateOfRow})
                                  </button>
                                  <button
                                    onClick={() =>
                                      selectResolutionChoice(
                                        anomaly,
                                        "keep",
                                        `Approved double ledger entry (keep both row ${row.rowNumber} and row ${anomaly.duplicateOfRow})`,
                                      )
                                    }
                                  ...
                                  <button
                                    onClick={() =>
                                      selectResolutionChoice(
                                        anomaly,
                                        "keep_this",
                                        `Kept current Row ${row.rowNumber} and marked Row ${anomaly.duplicateOfRow} for deletion`,
                                      )
                                    }
                                    className="px-3.5 py-1.5 bg-brand-light hover:bg-brand/10 border border-brand rounded text-xs font-semibold text-brand transition-colors cursor-pointer"
                                  >
                                    Keep Row {row.rowNumber} (Discard Row{" "}
                                    {anomaly.duplicateOfRow})
                                  </button>
                                  <button
                                    onClick={() =>
                                      selectResolutionChoice(
                                        anomaly,
                                        "keep_both",
                                        `Approved both conflicting duplicate records`,
                                      )
                                    }
                                    className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded text-xs font-semibold text-slate-700 transition-colors cursor-pointer"
                                  >
                                  ...
                                  <button
                                    onClick={() =>
                                      selectResolutionChoice(
                                        anomaly,
                                        "dmy",
                                        `Resolved ambiguous date. Assumed DD-MM-YYYY format: May 4, 2026`,
                                      )
                                    }
                                    className="px-3.5 py-1.5 bg-brand-light hover:bg-brand/10 border border-brand rounded text-xs font-semibold text-brand transition-colors cursor-pointer"
                                  >
                                  ...
                                  <button
                                    onClick={() => {
                                      const parsed = new Date(
                                        Date.UTC(2026, 3, 5, 12, 0, 0),
                                      ); // April 5th
                                      selectResolutionChoice(
                                        anomaly,
                                        "mdy",
                                        `Resolved ambiguous date. Switch to MM-DD-YYYY format: April 5, 2026`,
                                ...
                                <button
                                  onClick={() =>
                                    selectResolutionChoice(
                                      anomaly,
                                      "confirm",
                                      `Normalized date string "${row.originalDateText}" to standard ISO Date`,
                                    )
                                  }
                                  ...
                                  <button
                                    onClick={() =>
                                      selectResolutionChoice(
                                        anomaly,
                                        "inr",
                                        `Applied default group currency (INR)`,
                                      )
                                    }
                                    className="px-3.5 py-1.5 bg-brand-light hover:bg-brand/10 border border-brand rounded text-xs font-semibold text-brand transition-colors cursor-pointer"
                                  >
                                  ...
                                  <button
                                    onClick={() =>
                                      selectResolutionChoice(
                                        anomaly,
                                        "usd",
                                        `Assigned USD currency to transaction`,
                                        { currency: "USD", amount: row.amount },
                                      )
                                    }
                                    className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded text-xs font-semibold text-slate-700 transition-colors cursor-pointer"
                                  >
                              ...
                              {(anomaly.type === "MISSING_PAYER" ||
                                anomaly.type === "INVALID_PAYER" ||
                                anomaly.type === "USER_NAME_ALIAS") && (
                                    ...
                                    <button
                                      ...
                                      onClick={() =>
                                        selectResolutionChoice(
                                          anomaly,
                                          u.id,
                                          `Mapped payer alias "${row.paidBy}" to group user account "${u.username}"`,
                                          {
                                            paidById: u.id,
                                            paidBy: u.username,
                                          },
                                ...
                                <button
                                  onClick={() =>
                                    selectResolutionChoice(
                                      anomaly,
                                      "refund",
                                      `Approved negative balance line entry as group refund`,
                                    )
                                  }
                                  className="px-3.5 py-1.5 bg-brand hover:bg-brand-dark rounded text-xs font-semibold text-white transition-colors cursor-pointer"
                                >
                                ...
                                <button
                                  onClick={() =>
                                    selectResolutionChoice(
                                      anomaly,
                                      "settle",
                                      `Re-classified transaction as direct debt settlement payment`,
                                    )
                                  }
                                  className="px-3.5 py-1.5 bg-brand hover:bg-brand-dark rounded text-xs font-semibold text-white transition-colors cursor-pointer"
                                >
                                ...
                                <button
                                  onClick={() =>
                                    selectResolutionChoice(
                                      anomaly,
                                      "round",
                                      `Rounded fractional currency decimals to ₹${row.amount}`,
                                    )
                                  }
                                  className="px-3.5 py-1.5 bg-brand hover:bg-brand-dark rounded text-xs font-semibold text-white transition-colors cursor-pointer"
                                >
                              ...
                              {anomaly.type === "INACTIVE_MEMBER_SPLIT" && (
                                <>
                                  <button
                                    onClick={() => {
                                      const inactiveIds =
                                        anomaly.inactiveUserIds || [];
                                      // splitWith may be a string like "Aisha;Rohan" or an array of IDs
                                      const splitArr = Array.isArray(
                                        row.splitWith,
                                      )
                                        ? row.splitWith
                                        : (row.splitWith || "")
                                            ...
                                            .map((s) => s.trim())
                                            .filter(Boolean);
                                      const filteredSplit = splitArr.filter(
                                        (uid) => !inactiveIds.includes(uid),
                                      );
                                      const inactiveNames = inactiveIds
                                        .map(
                                          (uid) =>
                                      ...
                                      selectResolutionChoice(
                                        anomaly,
                                        "recalculate",
                                        `Excluded inactive members (${inactiveNames.join(", ")}) & recalculated splits`,
                                        { splitWith: filteredSplit.join(";") },
                                      );
                                    }}
                                    className="px-3.5 py-1.5 bg-brand hover:bg-brand-dark rounded text-xs font-semibold text-white transition-colors cursor-pointer"
                                  >
                                  ...
                                  <button
                                    onClick={() =>
                                      selectResolutionChoice(
                                        anomaly,
                                        "keep_anyway",
                                        `Enforced split calculation containing inactive members (kept in split list)`,
                                      )
                                    }
                                    className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded text-xs font-semibold text-slate-700 transition-colors cursor-pointer"
                                  >
                                  ...
                                  <button
                                    onClick={() => {
                                      selectResolutionChoice(
                                        anomaly,
                                        "absorb",
                                        `Absorbed unregistered member Kabir's cost split share into Payer Dev's ledger debit account`,
                                        {},
                                      );
                                    }}
                                    className="px-3.5 py-1.5 bg-brand hover:bg-brand-dark rounded text-xs font-semibold text-white transition-colors cursor-pointer"
                                  ...
                                  <button
                                    onClick={() =>
                                      selectResolutionChoice(
                                        anomaly,
                                        "create_guest",
                                        `Created dynamic guest user profile for "${anomaly.unregisteredName}"`,
                                      )
                                    }
                                    className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded text-xs font-semibold text-slate-700 transition-colors cursor-pointer"
                                  >
                                  ...
                                  <button
                                    onClick={() =>
                                      selectResolutionChoice(
                                        anomaly,
                                        "discard",
                                        `Discarded zero-amount transaction Row ${row.rowNumber}`,
                                        { isExcluded: true },
                                      )
                                    }
                                    className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded text-xs font-semibold text-slate-700 transition-colors cursor-pointer"
                                  ...
                                  <button
                                    onClick={() =>
                                      selectResolutionChoice(
                                        anomaly,
                                        "keep",
                                        `Retained zero-amount ledger row`,
                                      )
                                    }
                                    className="px-3.5 py-1.5 bg-brand-light hover:bg-brand/10 border border-brand rounded text-xs font-semibold text-brand transition-colors cursor-pointer"
                                  >
                ...
                <div className="bg-slate-100 rounded-xl p-4 space-y-2 border border-slate-200 max-h-48 overflow-y-auto">
                  {anomalies
                    ...
                    .map((a) => (
                      <div
                        key={a.id}
                        className="text-xs flex justify-between items-center text-slate-600 bg-white p-2.5 rounded shadow-xs"
                      >
                        <span>
